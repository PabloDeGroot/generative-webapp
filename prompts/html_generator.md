...
- Real images come first: when the spec or data gives an image URL (artwork, photo, portrait — usually a relative `/__art/...` path), use exactly that URL. Only when no real image exists, use a descriptive relative URL (e.g. /images/<topic>/<filename>.png), which is generated by AI; never use a generated image to stand in for a real artwork, person or place that has one.
