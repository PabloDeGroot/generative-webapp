...
14. Keep prop values in private fields (e.g. `this._title`, `this._items`), never on built-in element properties. Every HTMLElement already has properties that reflect attributes — `title`, `id`, `lang`, `dir`, `hidden`, `slot`, `className`, `tabIndex`, `role`, `draggable`, `inert`, and others — and assigning one (`this.title = newValue` in `attributeChangedCallback`) sets the attribute again, re-triggering the callback forever.
