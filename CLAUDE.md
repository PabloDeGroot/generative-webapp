...
- **Components get their data through the action runner:** a component displaying an entity takes its identity as a short attribute and fetches the rest on mount (POST with an `intent` and `outputFormat`), never a JSON prop. `HandleAction` caches responses whose tools were all read-only (`src/lib/server/action-cache.ts`, Firestore `toolkits/{id}/actionCache`, 15 min, response header `X-Action-Cache`); any successful write bumps the toolkit's cache version, invalidating its cached reads on every instance. Tools must throw on failure (not return `found: false`) so failures aren't cached: the MCP server turns a throw into an `isError` result, which `runMcpToolLoop` records as a failed call. A run whose write failed (or whose every call failed) answers `422 {ok:false, message}` whatever the model wrote, and ids/URLs the model mis-copied from tool results are repaired (`repairCopiedStrings`).
- **Real images first:** pages must not point at external hosts, so real images (museum artwork, Wikimedia photos) go through the allowlisted same-origin proxy `src/routes/__art/[source]/[...path]/+server.ts` (`/__art/aic|met|wiki/...`; `hooks.server.ts` keeps these paths away from AI image generation). Toolkit tools return those relative URLs; AI-generated `/images/*.png` routes are only for things with no real image.
