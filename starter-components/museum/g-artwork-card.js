...
// Card-specific styles. The mat pulses (the kit's skeleton) until its image has loaded or failed.
const CARD_CSS = `
...
figure { display: flex; flex-direction: column; flex: 1; margin: 0; min-width: 0; }
.mat { aspect-ratio: 4 / 5; width: 100%; border-radius: 2px; transition: box-shadow .2s ease; }
.mat.wait { animation: sk 1.6s ease-in-out infinite; }
/* Out of grid sizing (like .ph): as a grid item a tall portrait image would grow the row past the 4:5 mat and be clipped. */
.mat img { position: absolute; inset: 0; }
figcaption { display: grid; gap: 2px; align-content: start; padding: 12px 2px 4px; min-width: 0; }
.ttl { margin: 0; font-size: 18px; line-height: 1.3; color: var(--ink); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; overflow-wrap: anywhere;
...
.who { margin: 2px 0 0; font: 500 14px/1.4 var(--sans); color: var(--ink); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.meta { margin: 0; font-size: 12px; line-height: 1.45; color: var(--soft); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.meta b { font-weight: 400; padding: 0 .35em; color: var(--rule); }
...
.small .ttl { font-size: 15px; }
.small .who { font-size: 13px; }
.small figcaption { padding-top: 10px; }
.small .ph i { font-size: 14px; }
@media (max-width: 559px) { .ttl { font-size: 16px; } .who { font-size: 13px; } .meta { font-size: 11.5px; } figcaption { padding-top: 10px; } .ph i { font-size: 14px; } }
@media (prefers-reduced-motion: reduce) { a.card:hover, a.card:focus-visible { transform: none; } }`;

class GArtworkCard extends HTMLElement {
  // artwork-title, not title: the global title attribute would add a native tooltip and an accessible
  // description to the whole card. A legacy title attribute is moved to artwork-title.
  static get observedAttributes() { return ["artwork-id", "artwork-title", "title", "artist", "date", "museum", "image-url", "image-large-url", "image-alt", "size"]; }
  constructor() {
    super();
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === "title") {
      if (newValue !== null) {
        if (!this.hasAttribute("artwork-title")) this.setAttribute("artwork-title", newValue);
        this.removeAttribute("title");
      }
      return;
    }
    // Upgrades fire this once per attribute before connectedCallback renders; later changes are batched
    // into one render per microtask.
    if (oldValue === newValue || !this._ready || this._queued) return;
    this._queued = true;
    queueMicrotask(() => {
      this._queued = false;
      if (this._ready && this.isConnected) this.render();
    });
  }
  render() {
    const attr = (name) => (this.getAttribute(name) || "").trim();
    const id = attr("artwork-id");
    const title = attr("artwork-title") || "Untitled";
    const artist = attr("artist");
    const small = attr("size") === "small";
    const meta = [attr("date"), attr("museum")].filter(Boolean);
    const alt = attr("image-alt") || (artist ? `${title} by ${artist}` : title);
    const img = artImg(attr("image-url"), alt, title, {
      large: attr("image-large-url"),
      sizes: small ? "(min-width: 900px) 200px, 45vw" : "(min-width: 1240px) 300px, (min-width: 900px) 25vw, (min-width: 520px) 45vw, 90vw"
    });
    const caption = `<figcaption>
        <p class="ttl t">${esc(title)}</p>
        ${artist ? `<p class="who">${esc(artist)}</p>` : ""}
        ${meta.length ? `<p class="meta num">${meta.map(esc).join(`<b aria-hidden="true">·</b>`)}</p>` : ""}
      </figcaption>`;
    const figure = `<figure><div class="mat">${img}</div>${caption}</figure>`;
    // Without a valid id there is no page to open, so the card is shown but not linked.
    const body = isArtworkId(id)
      ? `<a class="card" href="${esc(artworkHref(id))}" aria-label="${esc(artist ? `${title}, ${artist}` : title)}">${figure}</a>`
      : `<div class="card">${figure}</div>`;
    this.shadowRoot.innerHTML = `<style>${TOKENS}${CARD_CSS}</style><article class="${small ? "small" : ""} h-full">${body}</article>`;
    adoptKitStyle(this.shadowRoot);
    wireImages(this.shadowRoot);
    const pending = this.shadowRoot.querySelector(".mat img:not(.in)");
    if (pending) {
      const mat = pending.closest(".mat");
      const done = () => mat.classList.remove("wait");
      mat.classList.add("wait");
      pending.addEventListener("load", done, { once: true });
      pending.addEventListener("error", done, { once: true });
    }
  }
}
if (!customElements.get("g-artwork-card")) customElements.define("g-artwork-card", GArtworkCard);
