...
class GBoardTile extends HTMLElement {
  static get observedAttributes() { return ["board-id", "name", "description", "post-count", "last-activity"]; }
  constructor() {
    super();
  ...
  render() {
    const count = this.getAttribute("post-count");
    const last = this.getAttribute("last-activity");
    const desc = this.getAttribute("description");
    const activity = [count !== null ? `${count} post${count === "1" ? "" : "s"}` : "", last ? `active ${ago(last)}` : ""].filter(Boolean).join(" · ");
    this.shadowRoot.innerHTML = `<style>${TOKENS}
      :host { height: 100%; }
      a { display: grid; gap: 4px; align-content: start; height: 100%; text-decoration: none; background: var(--card); border: 1px solid var(--rule); border-radius: 8px; padding: 12px 14px; }
      a:hover { border-color: var(--pine); }
      b { font: 700 16px var(--display); }
      span { font-size: 13px; color: var(--soft); }
      small { font-size: 12px; color: var(--pine); font-weight: 700; }
    </style>
    <a href="/b/${encodeURIComponent(this.getAttribute("board-id") || "")}"><b>${esc(this.getAttribute("name"))}</b>${desc ? `<span>${esc(desc)}</span>` : ""}${activity ? `<small>${esc(activity)}</small>` : ""}</a>`;
  }
}
customElements.define("g-board-tile", GBoardTile);
