...
class GPageSection extends HTMLElement {
  static get observedAttributes() { return ["heading", "subheading"]; }
  constructor() {
    super();
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>${TOKENS}
      ...
      h2 { margin: 0; font: 700 24px/1.2 var(--display); text-wrap: balance; }
      p { margin: -6px 0 0; color: var(--soft); font-size: 14px; }
    </style>
    <section>${heading ? `<h2>${esc(heading)}</h2>` : ""}${sub ? `<p>${esc(sub)}</p>` : ""}<slot></slot></section>`;
  }
}
