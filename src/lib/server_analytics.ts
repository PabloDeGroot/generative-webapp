...
import { GA_MEASUREMENT_ID } from '$env/static/private';
...
export async function logServerSideEvent(eventName: string, parameters?: Record<string, any>) {
  ...
  try {
    const response = await fetch(`https://www.google-analytics.com/mp/collect?measurement_id=${GA_MEASUREMENT_ID}&api_secret=${env.GA_API_SECRET}`, {
      method: 'POST',
      headers: {
